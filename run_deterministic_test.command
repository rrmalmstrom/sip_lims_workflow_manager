#!/bin/bash
# TEST VERSION: SIP LIMS Workflow Manager Docker Runner with Deterministic Image
# This is a test version that uses the deterministic Docker image to validate the SQLAlchemy fix
...
production_auto_update() {
    echo "🧪 Test mode - using deterministic Docker image (no auto-updates)..."
    
    # Set up centralized scripts directory (but don't update anything)
    local scripts_dir="$HOME/.sip_lims_workflow_manager/scripts"
    
    # Skip all updates - just use existing scripts and deterministic image
    echo "⏭️  Skipping Docker image updates"
    echo "⏭️  Skipping script updates"
    ...
    # Use deterministic Docker image for testing
    export DOCKER_IMAGE="sip-lims-workflow-manager:deterministic"
    
    echo "📁 Using existing scripts: $SCRIPTS_PATH"
    echo "🐳 Using deterministic Docker image: $DOCKER_IMAGE"
}
...
select_development_script_path() {
    ...
    # Use deterministic Docker build for testing
    export DOCKER_IMAGE="sip-lims-workflow-manager:deterministic"
