"""
Integration tests for the undo/snapshot system — SPS-CE workflow fixture.

Tests the full core.py → SnapshotManager stack against a real on-disk project
folder using the SPS-CE workflow fixture and mock scripts.
...
The 13 scenarios are identical in structure to the Capsule integration tests
(tests/test_undo_system_integration.py) but use SPS-CE-specific step IDs,
directory names, FA archive paths, and SNAPSHOT_ITEMS.

Run with:
    pytest tests/test_undo_system_integration_sps.py -v
"""
...
# Import SPS-CE fixture generator
from tests.fixtures.generate_sps_fixture import (
    SPS_STEPS,
    create_sps_fixture,
    get_step_produces,
...
def simulate_step_run(
    ...
    """
    step_def = next(s for s in SPS_STEPS if s["id"] == step_id)
...
@pytest.fixture
def sps_project(tmp_path):
    """Fresh SPS-CE project with no steps completed."""
    project_path, checkpoints = create_sps_fixture(tmp_path, steps_to_complete=0)
    project = Project(project_path, script_path=MOCK_SCRIPTS_DIR)
    return project, checkpoints


@pytest.fixture
def sps_project_3_steps(tmp_path):
    """SPS-CE project with first 3 steps pre-completed."""
    project_path, checkpoints = create_sps_fixture(tmp_path, steps_to_complete=3)
    project = Project(project_path, script_path=MOCK_SCRIPTS_DIR)
    return project, checkpoints
...
def test_happy_path_multi_step(sps_project):
    """
    ...
    completion_order = project.state_manager.get_completion_order()
    assert completion_order == [s["id"] for s in SPS_STEPS[:3]]
...
def test_automatic_rollback_on_failure(sps_project):
    """
    ...
    simulate_step_run(project, "initiate_project", run_number=1, success=True)

    # Record state before step 2 runs
    pre_run_db = (project.path / "project_summary.db").read_text()

    # Run step 3 (read_wga_and_make_spits) but it FAILS
    # Step 3 has project_summary.db in SNAPSHOT_ITEMS — good for testing restore
    simulate_step_run(project, "read_wga_and_make_spits", run_number=1, success=False)

    # Step state must remain "pending" (not completed)
    assert project.get_state("read_wga_and_make_spits") == "pending"

    # The snapshot pair is consumed by rollback — no longer in .snapshots/
    assert not project.snapshot_manager.snapshot_exists("read_wga_and_make_spits", 1)
...
def test_manual_undo_single_step(sps_project):
    """
    Step completes; user undoes; project folder matches pre-run state.
    """
    project, checkpoints = sps_project
    step_id = "initiate_project"
    ...
    assert not (project.path / "project_summary.db").exists()
    assert not (project.path / "sample_metadata.csv").exists()
    assert not (project.path / "individual_plates.csv").exists()
...
def test_manual_undo_multi_step(sps_project):
    """
    ...
    assert result is True
    assert project.get_state("read_wga_and_make_spits") == "pending"
    assert project.get_state("process_wga_results") == "completed"

    # Undo step 2
    result = perform_undo(project)
    assert result is True
    assert project.get_state("process_wga_results") == "pending"
    assert project.get_state("initiate_project") == "completed"

    # Undo step 1
    result = perform_undo(project)
    assert result is True
    assert project.get_state("initiate_project") == "pending"
    ...
    assert not (project.path / "project_summary.db").exists()
    assert not (project.path / "sample_metadata.csv").exists()
    assert not (project.path / "master_plate_data.csv").exists()
...
def test_user_added_files_preserved_after_undo(sps_project):
    """
    ...
    simulate_step_run(project, "process_wga_results", run_number=1, success=True)
...
def test_user_added_files_not_deleted_on_rollback(sps_project):
    """
    ...
    simulate_step_run(project, "process_wga_results", run_number=1, success=False)
...
def test_rerunnable_step_undo(sps_project):
    """
    initiate_project (allow_rerun=True) run 3 times; undo once; run counter
...
def test_legacy_fallback(tmp_path):
    ...
    """
    project_path, _ = create_sps_fixture(tmp_path, steps_to_complete=0)
    project = Project(project_path, script_path=MOCK_SCRIPTS_DIR)
    ...
    sentinel_content = "legacy sps db content\n"
...
def test_missing_snapshot_items_aborts_run_step(tmp_path):
    ...
    # Use the NO_SNAPSHOT_ITEMS mock script for the first_fa_analysis step
    no_items_dir = tmp_path / "no_items_scripts"
    no_items_dir.mkdir()

    src = MOCK_SCRIPTS_DIR / "SPS_first_FA_output_analysis_NEW_NO_SNAPSHOT_ITEMS.py"
    dst = no_items_dir / "SPS_first_FA_output_analysis_NEW.py"
    shutil.copy(src, dst)

    project = Project(project_path, script_path=no_items_dir)

    # Pre-complete steps 1–4 so step 5 (first_fa_analysis) is next
    for step in SPS_STEPS[:4]:
        simulate_step_run(project, step["id"], run_number=1, success=True)
...
def test_fa_archive_permanent_protection(sps_project):
    """
    FA analysis step runs and fails; FA archive subfolders are NOT deleted
    during rollback (PERMANENT_EXCLUSIONS protection).

    SPS-CE uses archived_files/first_lib_attempt_fa_results/ as the FA archive path.
    """
    project, _ = sps_project
    ...
    # SPS-CE uses the legacy path (first_lib_attempt_fa_results)
    fa_archive = project.path / "archived_files" / "first_lib_attempt_fa_results"
    fa_archive.mkdir(parents=True, exist_ok=True)
    archive_file = fa_archive / "old_fa_results.csv"
    archive_file.write_text("archived SPS FA data — must not be deleted\n")
    ...
    # Run step 2 but it FAILS — rollback must not touch PERMANENT_EXCLUSIONS
    simulate_step_run(project, "process_wga_results", run_number=1, success=False)
...
def test_manifest_diff_detects_newly_added_files(sps_project):
    """
    ...
    # Run step 2 — manifest diff should detect user files as newly-added
    step_id = "process_wga_results"
    run_number = 1

    # Find previous manifest
    prev_manifest_path = (
        project.snapshot_manager.snapshots_dir / "initiate_project_run_1_manifest.json"
    )
...
def test_empty_dirs_from_previous_step_survive_rollback(sps_project):
    """
    Step 1 creates empty directories as part of the SPS-CE project folder structure:
      2_sort_plates_and_amplify_genomes/, 3_make_library_analyze_fa/, 4_pooling/,
      archived_files/, MISC/
    ...
    # Complete step 1 — creates project folder structure including empty subdirs
    simulate_step_run(project, "initiate_project", run_number=1, success=True)

    # The simulate_step_run helper creates the empty dirs via creates_empty_dirs.
    # Verify they exist and are empty before step 2 runs.
    empty_dirs = [
        project.path / "2_sort_plates_and_amplify_genomes",
        project.path / "3_make_library_analyze_fa",
        project.path / "4_pooling",
    ...
    assert not project.snapshot_manager.snapshot_exists("process_wga_results", 1)
